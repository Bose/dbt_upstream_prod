    ...
    *******************/
    ...
    {% for test in selected_tests %}
        ...
        {% for test_ref in test_node.refs %}
            -- Only add the test to selected nodes if the parent relation exists
            -- This prevents scenarios of tests from dbt_utils or dbt_expectations to assert relations consistency (e.g: equal_rowcount)
            -- where the parent relation from the target environment is not materialized, and in this case should use the prod relation
            {% if test_ref.name == parent_model and load_relation(parent_ref) is not none %}
                {% if parent_project is none %}
